              ...
              <template v-slot:top>
                <v-card-title flat>
                  ...
                  <v-btn
                    class="ml-5"
                    @click="
                      $router.push('/user/services/add-secure-tax-sendoff')
                    "
                    color="#ffb300"
                  >
                    Add Secure Tax Send-off
                  </v-btn>
                  <v-btn
                    class="ml-5"
                ...
                <v-stepper
                  v-model="step"
                  :alt-labels="$vuetify.breakpoint.xs ? true : false"
                  color="#ffb300"
                >
                  <!-- Step 1 -->
                  <v-stepper-header>
                    <v-stepper-step
                      step="1"
                      @click="goToStep('/terms')"
                      style="cursor: pointer"
                      complete
                      color="#ffb300"
                    >
                      1. Fill Tax Form Navigator
                    </v-stepper-step>
                    ...
                    <v-stepper-step
                      step="3"
                      @click="goToStep('/user/services/add')"
                      style="cursor: pointer"
                      complete
                      color="#ffb300"
                    >
                    ...
                    <!-- Step 4 -->
                    <v-stepper-step step="4" complete color="#ffb300">
                      4. Wait for Our Response
                    </v-stepper-step>
                  </v-stepper-header>

                  <!-- Stepper Content (Optional) -->
                  <v-stepper-items>
  ...
  methods: {
    ...
    addNote() {
      try {
        ...
        const payload = { notes: this.newNote };
        this.$api.premierService
          .updateNotes(this.itemId, payload)
          .then((res) => {
            if (res?.data) {
              this.newNote = "";
    ...
    viewDetails(item) {
      if (item?.service_id !== "3") {
        ...
        this.$router.push({
          path: "/user/services/view-secure-tax-send-off",
          query: {
            id: item?.id,
          },
        });
    ...
    async handlePayNow(item) {
      ...
      if (item.total_price === "1.00") {
        payload.label = "test_payment";
        payload.payment_service = "Test";
      }
      ...
      try {
        ...
        const res = await this.$api.premierService.userPaymentCheckout(payload);
        if (res && res.data) {
