<template>
  <div class="pb-6">
    <v-row>
      <v-col cols="12" md="3">
        <button
          ...
          @click="$router.push('/admin/liason-call')"
        >
          Back
        </button></v-col
      >
    ...
    <form @submit.prevent="handleSubmit">
      <div style="display: flex; justify-content: right" v-if="!isEditing">
          ...
          <tbody>
            ...
            >
              ...
              <td>
                ...
                >
                  <option value="" :disabled="!isEditing">Select</option>
      ...
      <div class="form-group">
        ...
        <select
          ...
          class="form-control"
        >
          <option value="" :disabled="!isEditing" selected>Select Type</option>
...
export default {
  ...
  methods: {
    ...
    async getLiaisonByAdminId() {
      this.loading = true;
      try {
        this.$api.premierService
          .getLiaisonByAdminId(this.$route.query.id)
          .then((res) => {
            if (res?.data) {
              console.log(res);
              this.form = res?.data;
    ...
    async handleSubmit() {
      ...
      try {
        this.$api.premierService
          .updateLiaisonForm(this.form?.id, this.form)
          .then((res) => {
            if (res?.data) {
              this.$swal.fire({
                toast: true,
